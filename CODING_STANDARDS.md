...
예시:
✅ Feat: 캘린더 네비게이션 훅 추가
✅ Refactor: 네비게이션 로직을 커스텀 훅으로 추출
✅ Fix: 주간 뷰에서 일정 겹침 문제 해결
✅ Feat: 다크모드 구현 [WIP]
```
...
**상태 표시 (선택사항):**
- 작업 중: `[WIP]` (Work In Progress) - 제목 끝에 추가
- 완료: 상태 표시 없음
...
```markdown
❌ 제목: 업데이트  (타입 없음, 내용 불명확)
❌ 제목: 캘린더 수정  (타입 없음)
❌ 제목: Fix  (내용 설명 없음)
❌ 개요: 캘린더 수정  (이유 불명확)
❌ 작업한 내용: 여러 파일 변경  (구체적이지 않음)
❌ 리뷰 가이드: (비어있음)
❌ 테스트 결과: 잘 됨  (구체적이지 않음)
```
