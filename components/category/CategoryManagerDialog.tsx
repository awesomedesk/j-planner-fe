...
import { CATEGORY_NAME_MAX_LENGTH, getMoveAfterId } from './utils/categoryUtils';
...
/**
 * CategoryManagerDialog - 카테고리 관리 창 (OV-04, US-04)
...
export default function CategoryManagerDialog({ onClose }: CategoryManagerDialogProps) {
  ...
  return (
    <DialogFrame
      ...
      footer={
        <>
          ...
          <ThemeButton size="md" onClick={onClose}>
            닫기
          </ThemeButton>
        </>
      }
    >
      ...
      {categories.length > 1 && <p className="text-right text-xs text-tp-muted">▲▼로 순서 변경. 이 순서가 필터 드롭다운·입력 창 목록 순서</p>}
      ...
      >
        ...
        <ColorPicker label="새 카테고리 색" value={manager.newColor} onChange={manager.setNewColor} />
        {manager.addError && (
...
const ROW_CLASS = 'flex min-h-[46px] items-center gap-2.5 rounded-[10px] border border-tp-line px-3 py-2';

function ColorDot({ color }: { color: HexColor }) {
  return <span className="h-[18px] w-[18px] shrink-0 rounded-full" style={{ backgroundColor: color }} aria-hidden="true" />;
}

function CountText({ category }: { category: Category }) {
  return (
    <span className="whitespace-nowrap text-xs text-tp-muted">
...
function DefaultCategoryRow({ category }: { category: Category }) {
  return (
    <div className={`${ROW_CLASS} bg-tp-panel`}>
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-0.5 fold:flex-row fold:items-center fold:gap-2.5">
        <span className="min-w-0 flex-1 truncate text-sm font-semibold">{category.name}</span>
        <CountText category={category} />
      </div>
      <span className="shrink-0 rounded-full border border-tp-secondary-line bg-tp-bg px-2 py-0.5 text-[11px] font-bold text-tp-on-secondary">
        기본 · 맨 위 고정
      </span>
    </div>
  );
...
function CategoryRow({ category, manager }: { category: Category; manager: Manager }) {
    ...
    return (
      <div className={`${ROW_CLASS} flex-wrap bg-tp-bg`}>
        ...
        <span className="min-w-0 flex-1 text-sm">
          <b className="font-semibold">{category.name}</b>을(를) 지울까요? 일정 {category.scheduleCount} · Todo {category.todoCount}는 미지정으로
          옮겨져요.
        </span>
        <div className="flex gap-1.5">
          ...
          <ThemeButton onClick={manager.cancelDelete}>그만두기</ThemeButton>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={`${ROW_CLASS} bg-tp-bg`}>
      ...
      <ThemeButton variant="danger" className="px-1.5" onClick={() => void manager.deleteCategory(category)} disabled={isBusy}>
        삭제
      </ThemeButton>
      <span className="inline-flex gap-0.5">
        <ThemeButton iconOnly className="!h-7 !w-7" aria-label={`${category.name} 위로`} disabled={!canMoveUp || isBusy} onClick={() => void manager.moveCategory(category, 'up')}>
          <Icon name="chevronUp" size={14} strokeWidth={2.4} />
        </ThemeButton>
        <ThemeButton iconOnly className="!h-7 !w-7" aria-label={`${category.name} 아래로`} disabled={!canMoveDown || isBusy} onClick={() => void manager.moveCategory(category, 'down')}>
...
function EditingRow({ manager }: { manager: Manager }) {
  const isBusy = manager.busyId === manager.editingId;
  return (
    <form
    ...
    >
      <div className="flex items-center gap-2">
        <ColorDot color={manager.editColor} />
        <input
          type="text"
...
function ColorPicker({ label, value, onChange }: { label: string; value: HexColor; onChange: (color: HexColor) => void }) {
  return (
